...
class IndividualActorsTimelineScreen extends StatefulWidget {
  const IndividualActorsTimelineScreen({
    Key? key,
  ...
  @override
  _IndividualActorsTimelineScreenState createState() => _IndividualActorsTimelineScreenState();
}

class _IndividualActorsTimelineScreenState extends State<IndividualActorsTimelineScreen> with TickerProviderStateMixin {
  ///VARIABLES USED TO RETRIEVE AND FILTER THROUGH DATA FROM BACKEND
  late List<PoliticalMappingIndividualModel> readJsonFileContent =
  <PoliticalMappingIndividualModel>[];
  late List<PoliticalMappingIndividualModel> searchData =
  <PoliticalMappingIndividualModel>[];
  ...
  readPoliticalIndividualsFromJsonFile() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        readJsonFileContent =
            politicalMappingIndividualModelFromJson(response.body); //(jsonString);
        //print("ALL  individuals: $readJsonFileContent");

        if (initialStartDate == "2022" &&
            initialEndDate == "----") {
          searchData = readJsonFileContent
              .where((PoliticalMappingIndividualModel partyGroupActor) =>
          DateFormat("yyyy")
                            ...
                            dropDownState(() {
                              ...
                              if (initialStartDate == "----") {
                                setState(() {
                                  ...
                                  searchData = readJsonFileContent
                                      .where(
                                          (PoliticalMappingIndividualModel partyGroupActor) =>
                                      DateFormat("yyyy")
                            ...
                            dropDownState(() {
                              ...
                              if (initialEndDate == "----") {
                                setState(() {
                                  ...
                                  searchData = readJsonFileContent
                                      .where(
                                          (PoliticalMappingIndividualModel individual) =>
                                      DateFormat("yyyy")
                                          .format(DateTime.parse(
          ...
          builder: (context, data) {
            if (data.hasError) {
              ...
              return SizedBox(
                ...
                child: individualsTimelineModel(),
              );
            }
          },
        ),
  ...
  individualsTimelineModel() => Timeline.builder(
    itemBuilder: centerIndividualsTimelineBuilder,
    itemCount: searchData.length,
    physics: const NeverScrollableScrollPhysics(),
    position: TimelinePosition.Center,
  );

  TimelineModel centerIndividualsTimelineBuilder(BuildContext context, int i) {
        ...
        child: InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => PoliticalMappingIndividualActorDetailScreen(selectedIndividual: searchData[i]),
              ),
            );
          },
          child: Card(
