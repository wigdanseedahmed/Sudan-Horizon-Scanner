...
class InnovationAndDigitizationScreen extends StatefulWidget {
  const InnovationAndDigitizationScreen({Key? key}) : super(key: key);

  @override
  _InnovationAndDigitizationScreenState createState() => _InnovationAndDigitizationScreenState();
}

class _InnovationAndDigitizationScreenState extends State<InnovationAndDigitizationScreen> with TickerProviderStateMixin {
  ///VARIABLES USED TO RETRIEVE AND FILTER THROUGH DATA FROM BACKEND
  late List<InterventionsProjectModel> readJsonFileContent = <InterventionsProjectModel>[];
  late List<InterventionsProjectModel> _allIDProjects = <InterventionsProjectModel>[];
  ...
  Future<List<InterventionsProjectModel>> readProjectFromJsonFile() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        _allIDProjects = readJsonFileContent
  ...
  void _runFilter({required String enteredKeyword}) {
    ...
    if (enteredKeyword.isEmpty) {
      ...
      results = _allIDProjects;
    } else {
      // we use the toLowerCase() method to make it case-insensitive
      _allIDProjects.forEach((data) {
        if (data.projectName
            .toString()
            .toLowerCase()
                ...
                child: RichText(
                  text: const TextSpan(
                    ...
                    children: <TextSpan>[
                      TextSpan(
                          text:
                          "Digital technologies have the potential to boost more inclusive and sustainable growth by spurring innovation, generating efficiencies and improving services.",
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      TextSpan(
                          text:
                          " They are also an important tool in helping countries to confront and recover from the COVID-19 pandemic that has disrupted economies and societies globally. The OECD has been supporting the G20’s Digital agenda to help countries harness the benefits of the digital transformation, including job creation and business continuity, and access to critical health and education services, while addressing the potential challenges it may bring, by leveraging our cross-cutting and evidence-based analysis, and notably the OECD’s ongoing Going Digital project."),
                    ],
                  ),
                ),
              ),
                    ...
                    return Column(
                      children: [
                        ...
                        BuildInterventionsProjectList(
                            ...
                            theme: "I_D",
                            status: "current", screenSize: screenSize,),
                        SizedBox(height: screenSize.height / 10),
                        StatusProjectHeading(
                            text: "Past Project", screenSize: screenSize),
                        BuildInterventionsProjectList(
                          allProjects: searchData, theme: "I_D", status: "past", screenSize: screenSize,),
                      ],
                    );
                  } else {
                    return const Center(
                                ...
                                onChanged: (String? newValue) {
                                  dropDownState(() {
                                    ...
                                    if (initialLocality == "Locality") {
                                      ...
                                      setState(() {
                                        searchData = _allIDProjects
                                            .where((InterventionsProjectModel project) =>
