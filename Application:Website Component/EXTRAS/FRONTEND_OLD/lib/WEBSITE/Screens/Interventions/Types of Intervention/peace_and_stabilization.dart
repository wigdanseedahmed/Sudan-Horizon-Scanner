...
class PeaceAndStabilizationScreen extends StatefulWidget {
  const PeaceAndStabilizationScreen({Key? key}) : super(key: key);

  @override
  _PeaceAndStabilizationScreenState createState() =>
      _PeaceAndStabilizationScreenState();
}

class _PeaceAndStabilizationScreenState
    extends State<PeaceAndStabilizationScreen> with TickerProviderStateMixin {
  ...
  late List<InterventionsProjectModel> _allPSProjects =
      <InterventionsProjectModel>[];
  ...
  Future<List<InterventionsProjectModel>> readProjectFromJsonFile() async {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        _allPSProjects = readJsonFileContent
            .where((element) => element.theme!.contains("D_E"))
            .toList();
        //print("ALL PS PROJECTS: _allPSProjects");
  ...
  void _runFilter({required String enteredKeyword}) {
    ...
    if (enteredKeyword.isEmpty) {
      ...
      results = _allPSProjects;
    } else {
      // we use the toLowerCase() method to make it case-insensitive
      _allPSProjects.forEach((data) {
        if (data.projectName
            .toString()
            .toLowerCase()
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                image: DecorationImage(
                  image:
                      AssetImage('assets/images/peace_and_stabilization.jpg'),
                  fit: BoxFit.cover,
                ),
              ),
            ),
          ...
          Positioned(
            ...
            child: RotatedBox(
              ...
              child: CustomPaint(
                ...
                painter: DrawPSColouredTriangle(),
              ),
            ),
          ),
          Positioned(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                image: DecorationImage(
                  image: AssetImage(
                    'assets/images/peace_and_stabilization_logo.png',
                  ),
                  fit: BoxFit.fitWidth,
                ),
              ),
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: const Text(
            peaceAndStabilizationTheContext,
            style: TextStyle(
              fontSize: 18,
              fontFamily: 'Montserrat',
      ...
      children: [
        ...
        Padding(
          padding: EdgeInsets.only(
              left: screenSize.width / 25, right: screenSize.width / 25),
          child: const Text(
            peaceAndStabilizationOurApproach,
            style: TextStyle(
              fontSize: 18,
              fontFamily: 'Montserrat',
              fontWeight: FontWeight.w700,
...
class DrawPSColouredTriangle extends CustomPainter {
  @override
...
class DrawPSWhiteTriangle extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    var path = Path();
    path.moveTo(size.width / 2, 0);
    ...
    path.close();
    canvas.drawPath(
        path, Paint()..color = const Color.fromRGBO(247, 247, 247, 1));
  }
