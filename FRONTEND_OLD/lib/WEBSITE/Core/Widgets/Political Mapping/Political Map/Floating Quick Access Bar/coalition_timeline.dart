...
class CoalitionTimelineScreen extends StatefulWidget {
  const CoalitionTimelineScreen({
    Key? key,
  ...
  @override
  _CoalitionTimelineScreenState createState() =>
      _CoalitionTimelineScreenState();
}

class _CoalitionTimelineScreenState extends State<CoalitionTimelineScreen>
    with TickerProviderStateMixin {
  ///VARIABLES USED TO RETRIEVE AND FILTER THROUGH DATA FROM BACKEND
  late List<PoliticalMappingCoalitionModel> readJsonFileContent =
      <PoliticalMappingCoalitionModel>[];
  late List<PoliticalMappingCoalitionModel> searchData =
      <PoliticalMappingCoalitionModel>[];

  Future<List<PoliticalMappingCoalitionModel>>
      readPoliticalCoalitionFromJsonFile() async {
    /// String to URI, using the same url used in the nodejs code
    var uri = Uri.parse(AppUrl.getPoliticalMappingCoalitions);
    ...
    if (response.statusCode == 200) {
      setState(() {
        readJsonFileContent = politicalMappingCoalitionModelFromJson(
            response.body); //(jsonString);
        //print("ALL  coalitions: $readJsonFileContent");

        if (initialStartDate == "2022" && initialEndDate == "----") {
          searchData = readJsonFileContent
              .where((PoliticalMappingCoalitionModel partyGroupActor) =>
                  DateFormat("yyyy")
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
                            ...
                            dropDownState(() {
                              ...
                              if (initialStartDate == "----") {
                                setState(() {
                                  ...
                                  searchData = readJsonFileContent
                                      .where((PoliticalMappingCoalitionModel
                                              partyGroupActor) =>
                                          DateFormat("yyyy")
        ...
        FutureBuilder(
          future: readPoliticalCoalitionFromJsonFile(),
  ...
  coalitionTimelineModel() => Timeline.builder(
        itemBuilder: centerCoalitionTimelineBuilder,
        itemCount: searchData.length,
        physics: const NeverScrollableScrollPhysics(),
        position: TimelinePosition.Center,
      );

  TimelineModel centerCoalitionTimelineBuilder(BuildContext context, int i) {
        ...
        child: InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => PoliticalMappingCoalitionDetailScreen(
                    selectedCoalition: searchData[i]),
              ),
            );
          },
          child: Card(
